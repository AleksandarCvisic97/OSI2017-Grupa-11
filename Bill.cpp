...
bool check_bill(std::ifstream& file, int format)
{
    ...
    int check_format(std::ifstream& file)
    {
        ...
        if(temp.compare("Kupac:")==0) // Poredjenje sadrzaja stringa temp i teksta u zagradi
        {
            ...
            std::stringstream s(line);
            line.erase(std::remove(line.begin(),line.end(),' '),line.end()); // Brisanje razmaka u ucitanoj liniji
            if(line.compare("Racun")==0)
                return 1;
            else if(line.compare("OSIMarket")==0)
                return 4;
            else return 0;
        }
        ...
        {
            std::cout<<"File extension:"; // Unos ekstenzije fajla,da bi se provjerilo da li je u pitanju excel fajl
